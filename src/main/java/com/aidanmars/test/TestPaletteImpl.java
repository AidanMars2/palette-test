...
public final class TestPaletteImpl implements Palette {
    ...
    public TestPaletteImpl(byte dimension, byte minBitsPerEntry, byte maxBitsPerEntry, byte directBits) {
    ...
    TestPaletteImpl(byte dimension, byte minBitsPerEntry, byte maxBitsPerEntry, byte directBits, byte bitsPerEntry) {
    ...
    public void getAll(EntryConsumer consumer) {
        if (bitsPerEntry == 0) {
            TestPalettes.getAllFill(dimension, count, consumer);
        } else {
    ...
    public void getAllPresent(EntryConsumer consumer) {
        if (bitsPerEntry == 0) {
            if (count != 0) TestPalettes.getAllFill(dimension, count, consumer);
        } else {
    ...
    public void set(int x, int y, int z, int value) {
        ...
        final int oldValue = TestPalettes.write(dimension(), bitsPerEntry, values, x, y, z, paletteIndex);
    ...
    public void replace(int oldValue, int newValue) {
        ...
        if (bitsPerEntry == 0) {
            ...
            if (isDirect()) {
                ...
                if (newPos < 0) {
                    if (oldValue == 0 || newValue == 0) {
                        final int count = TestPalettes.count(dimension, bitsPerEntry, values, oldIndex);
            ...
            this.values = TestPalettes.remap(dimension, bitsPerEntry, bitsPerEntry, values, v -> {
                if (v == oldIndex) {
    ...
    public void copyFrom(Palette source, int offsetX, int offsetY, int offsetZ) {
        ...
        final TestPaletteImpl sourcePalette = (TestPaletteImpl) source;
        ...
        final int dimensionBitCount = TestMathUtils.bitsToRepresent(dimension - 1);
        ...
        for (int y = minY; y <= maxY; y++) {
            ...
            for (int z = minZ; z <= maxZ; z++) {
                ...
                for (int x = minX; x <= maxX; x++) {
                    ...
                    final int oldValue = TestPalettes.write(dimension, bitsPerEntry, values, x, y, z, targetPaletteIndex);
    ...
    public void copyFrom(Palette source) {
        final var sourcePalette = (TestPaletteImpl) source;
    ...
    public void load(int[] palette, long[] values) {
        ...
        if (bpe > maxBitsPerEntry) {
            ...
            final AtomicInteger count = new AtomicInteger();
            this.values = TestPalettes.remap(dimension, bpe, directBits, values, true, v -> {
    ...
    public int count(int value) {
        ...
        return TestPalettes.count(dimension, bitsPerEntry, values, queryValue);
    }

    void recount() {
        if (bitsPerEntry != 0) {
            final int zeroCount = TestPalettes.count(dimension, bitsPerEntry, values, valueToPalettIndexOrDefault(0));
    ...
    public boolean compare(Palette p) {
        ...
        final TestPaletteImpl palette = (TestPaletteImpl) p;
        final int dimension = this.dimension();
    ...
    public Palette clone() {
        final var clone = new TestPaletteImpl(dimension, minBitsPerEntry, maxBitsPerEntry, directBits);
    ...
    private void updateAll(int[] paletteValues, @Nullable PaletteIndexMap newPaletteIndexMap) {
        ...
        if (newPaletteIndexMap == null) {
            ...
            bpe = (byte) TestMathUtils.bitsToRepresent(Math.max(minBitsPerEntry, newPaletteIndexMap.size() - 1));
        }
    ...
    private void downsizeWithPalette(PaletteIndexMap palette) {
        ...
        final byte newBpe = (byte) Math.max(TestMathUtils.bitsToRepresent(palette.size() - 1), minBitsPerEntry);
        if (newBpe >= bpe || newBpe > maxBitsPerEntry) return;

        if (isDirect()) {
            this.values = TestPalettes.remap(dimension, bpe, newBpe, values, palette::valueToIndexOrDefault);
        } else {
            ...
            this.values = TestPalettes.remap(dimension, bpe, newBpe, values, value -> transform[value]);
        }
    ...
    void upsize() {
        ...
        if (newBpe > maxBitsPerEntry) {
            ...
            this.values = TestPalettes.remap(dimension, bpe, newBpe, values, Int2IntFunction.identity());
            this.bitsPerEntry = newBpe;
        }
    }
