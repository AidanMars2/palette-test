...
import static com.aidanmars.reference.Palettes.*;
...
public final class PaletteImpl implements Palette {
    ...
    public PaletteImpl(byte dimension, byte minBitsPerEntry, byte maxBitsPerEntry, byte directBits) {
        ...
        for (int y = minY; y <= maxX; y++) {
            for (int z = minZ; z < maxZ; z++) {
                for (int x = minX; x < maxX; x++) {
                    final int oldValue = Palettes.write(dimension, bitsPerEntry, values, x, y, z, paletteIndex);
                    if (oldValue == airPaletteIndex) countDelta++;
                }
            }
        }
    ...
    @Override
    public void replace(int x, int y, int z, java.util.function.IntUnaryOperator operator) {
        validateCoord(dimension, x, y, z);
